...
if len(sys.argv) > 1 and sys.argv[1] == "--subproc":
    ramp_list = [" ", "▁", "▂", "▃", "▄", "▅", "▆", "▇", "█"]
    ramp_list.extend(f'%{{F#{color.strip(" #")}}}█%{{F-}}' for color in sys.argv[2].split(",") if color)
    while True:
...
parser.add_argument("-f", "--framerate", type=int, default=60, help="Framerate to be used by cava, default is 60")
parser.add_argument("-b", "--bars", type=int, default=8, help="Amount of bars, default is 8")
parser.add_argument(
    "-e",
    "--extra_colors",
